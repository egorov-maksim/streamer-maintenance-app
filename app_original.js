...
function showSectionTooltip(e, cableId, sectionIndex) {
  ...
  // Build tooltip HTML
  let html = `
    <div class="tooltip-header">Streamer ${streamerNum}, Section ${sectionLabel}</div>
  `;
...
function isModulePosition(sectionIndex) {
  ...
  return (
    ...
    (sectionIndex > firstActive && sectionIndex < lastActive && (sectionIndex - firstActive) % moduleFreq === 0)
  );
}
...
async function deleteEvent(id) {
  ...
  const rangeLabel = `${formatAS(evt.section_index_start)} – ${formatAS(evt.section_index_end)}`;
  
  const confirmed = confirm(
    `🗑️ Delete Cleaning Event?\n\n` +
    `Streamer: ${streamerNum}\n` +
    `Sections: ${rangeLabel}\n` +
    `Method: ${evt.cleaning_method}\n` +
    `Date: ${formatDateTime(evt.cleaned_at)}\n\n` +
...
function exportCsv() {
  ...
  events.forEach((evt) => {
    ...
    rows.push([streamerNum, startSection, endSection, evt.cleaning_method, `"${dateStr}"`].join(","));
  });

  const csv = rows.join("\n");
  const textarea = safeGet("csv-output");
...
function parseCsvLine(line) {
  const regex = /,(?=(?:[^"]*"[^"]*")*[^"]*$)/;
  return line.split(regex).map((p) => p.trim());
}
...
function renderLog() {
  ...
  events.forEach((evt) => {
    ...
    const streamerNum = toStreamerNum(evt.cable_id);
    const rangeLabel = `${formatAS(evt.section_index_start)} – ${formatAS(evt.section_index_end)}`;
    const channelRange = `${getChannelRange(evt.section_index_start)} – ${getChannelRange(evt.section_index_end)}`;
...
/* ------------ Streamer Cards ------------ */
async function renderStreamerCards() {
  ...
  try {
    ...
    const totalPerCable = N + tailSections; // active + tail
    ...
    for (let c = 0; c < cableCount; c++) {
      ...
      const avgCleanings =
        cleanedCount > 0 ? (totalCleanings / cleanedCount).toFixed(1) : 0;
...
async function renderHeatmap() {
  ...
  try {
    ...
    for (let s = 0; s < N; s++) {
      ...
      // Module positioning:
      // - First module after AS1 (s === 0)
      // - Regular modules: every moduleFreq after first
      // - ALWAYS after last active section (s === N - 1)
      const sectionNumber = s + 1;
      const isFirstModule = sectionNumber === 1;
      const isRegularModule = 
        (sectionNumber > 1) &&
        ((sectionNumber - 1) % moduleFreq === 0);

      const isLastModule = sectionNumber === N;  // ALWAYS after last active
    ...
    for (let t = 0; t < tailSections; t++) {
      ...
      const startCh = (N + t) * channelsPerSection + 1;
      const endCh = startCh + channelsPerSection - 1;
      tailChannelCell.textContent = `${startCh}-${endCh}`;
      tailChannelCell.title = `Tail Channels ${startCh}–${endCh}`;
    ...
    for (let c = 0; c < cableCount; c++) {
      ...
      for (let s = 0; s < N; s++) {
        ...
        if (lastDate) {
          days = Math.floor(
            (Date.now() - new Date(lastDate)) / (1000 * 60 * 60 * 24)
          );
        }
        ...
        cell.title = lastDate
          ? `AS${String(s + 1).padStart(2, "0")}\nCh ${s * channelsPerSection + 1}–${
              (s + 1) * channelsPerSection
            }\nLast cleaned: ${formatDateTime(lastDate)} (${days} days ago)`
          : `AS${String(s + 1).padStart(2, "0")}\nCh ${s * channelsPerSection + 1}–${
              (s + 1) * channelsPerSection
            }\nNever cleaned`;
        ...
        // Module positioning:
        // - First module after AS1
        // - Regular modules every moduleFreq
        // - ALWAYS after last active section
        const sectionNumber = s + 1;
        const isFirstModule = sectionNumber === 1;
        const isRegularModule = 
          (sectionNumber > 1) &&
          ((sectionNumber - 1) % moduleFreq === 0);
        const isLastModule = sectionNumber === N;  // ALWAYS after AS107
...
function clearDragState() {
  ...
  dragState = { active: false, cableId: null, start: null, end: null, cells: null };
}
...
async function confirmCleaning() {
  ...
  if (
    isNaN(streamerNum) || streamerNum < 1 || streamerNum > config.numCables ||
    isNaN(startSection) || startSection < 1 || startSection > maxSection ||
    isNaN(endSection) || endSection < 1 || endSection > maxSection
  ) {
...
async function refreshStatsFiltered() {
  ...
  try {
    ...
    const res = await fetch(`/api/stats/filter?${params.toString()}`);
    const data = await res.json();

    const sectionsPerCable = config.sectionsPerCable + (config.useRopeForTail ? 0 : 5);
    const totalSections = config.numCables * sectionsPerCable;
    const cleanedSections = Math.min(data.totalDistance / (config.sectionLength || 1), totalSections);
    const coverage = totalSections > 0 ? ((cleanedSections / totalSections) * 100).toFixed(1) : 0;

    safeGet("kpi-coverage").textContent = `${coverage}%`;
    safeGet("kpi-coverage-sub").textContent = `${Math.floor(cleanedSections)} / ${totalSections} sections`;
    ...
    if (breakdownDiv && Object.keys(data.byMethod).length > 0) {
